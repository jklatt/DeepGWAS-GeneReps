...
def test(PATH):
    ...
    # plt.show()
    SAVING_PATH=os.getcwd()+'/plots_bedreader_leakyrelu_reduceplateu_lr{}_twostep_MLP_upsampling_attweight_{}_weightedfix_variousNsnp/'.format(args.lr,args.model)+ str(args.seed)
    os.makedirs(SAVING_PATH, exist_ok=True)

    EVALUATION_SAVINGPATH=os.getcwd()+'/metrics_bedreader_leakyrelu_reduceplateu_lr{}_twostep_MLP_upsampling_attweight_{}_weightedfix_variousNsnp/'.format(args.lr,args.model)+ str(args.seed)
...
if __name__ == "__main__":
    ...
    PATH=working_dir+'/checkpoints_bedreader_leakyrelu_reduceplateu_lr{}_twostep_MLP_upsampling_attweight_{}_weightedfix_variousNsnp/'.format(args.lr,args.model)+ str(args.seed)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        os.makedirs("./tensorboard_logs_bedreader_leakyrelu_reduceplateu_lr{}_twostep_MLP_upsampling_attweight_{}_weightedfix_variousNsnp/".format(args.lr,args.model)+ str(args.seed), exist_ok=True)
        writer = SummaryWriter('./tensorboard_logs_bedreader_leakyrelu_reduceplateu_lr{}_twostep_MLP_upsampling_attweight_{}_weightedfix_variousNsnp/'.format(args.lr,args.model)+ str(args.seed)+'/nsnp{}_max{}_csnp{}_i{}_prevalence{}'.format(args.num_snp,args.max_present,args.num_casual_snp,args.interaction,args.prevalence))
