...
def compute_per_gene_loop(details , num, chromosome,gene_list, gene,bed_mat, pos):
    gene_by_sample_prop=np.nan
...
Xt = Parallel(n_jobs=2,backend=multiprocessing)(delayed(compute_per_gene_loop)(details , num, chromosome,gene_list, gene,bed_mat, pos) for gene in gene_list[:8])
print(Xt)
