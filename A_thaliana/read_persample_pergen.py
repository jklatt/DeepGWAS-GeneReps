...
for maf in [0.01, 0.05, 0.1]:
	for chr in [1, 2, 3, 4, 5]:
		...
		with open('/home/zixshu/DeepGWAS/DeepGWAS-GeneReps/A_thaliana/max_present_stat/minor_present_percentage_{}MAF_chr{}.pkl'.format(maf,chr), 'rb') as f:
			minor_present=pickle.load(f)

		for idx, gene_allsamples in enumerate(minor_present):
			present_avg=np.mean(gene_allsamples)
...
num_snp_vs_present_avg.to_csv("/home/zixshu/DeepGWAS/DeepGWAS-GeneReps/A_thaliana/num_snp_vs_present_avg.csv")
...
plt.savefig('/home/zixshu/DeepGWAS/DeepGWAS-GeneReps/A_thaliana/num_snp_vs_present_avg.png')
